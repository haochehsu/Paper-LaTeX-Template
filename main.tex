...
\begin{enumerate}
    \item Seller chooses $p$.
    \item Intermediary observes seller's pricing decision and announces the disclosure policy $G$.
    \item Buyer observes both the price $p$ and the disclosure policy $G$. Buyer's private search cost $\xi$ realizes according to $H$. She will visit the intermediary if and only if $W_G(p)\geq s$. Otherwise, he does not visit and no purchase is made.
    \item When buyer pays his visit, his posterior expected value $s$ is drawn according to $G$. He purchases the product if $\E(v|s) \geq p$. Otherwise, no purchase is made.
\end{enumerate}
...
\hlabel{ch1_proof}We will prove the following large frac $\dfrac{1}{2}$ and inline frac $\tfrac{1}{2}$ equations:
\begin{proof}
